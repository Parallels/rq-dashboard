var url_for = function(name, param) {
    ...
    else if (name == 'scheduler_status') { url += 'status.json'; }
    return url;
};
...
var api = {
    ...
    getSchedulerStatus: function(cb) {
        $.getJSON(url_for('scheduler_status'), function(data) {
            cb(data);
        });
    }
};
    ...
    var onJobsLoaded = function(jobs, pagination, done) {
        ...
        if (jobs.length > 0) {
            $.each(jobs, function(i, job) {
                ...
                if (job.scheduled_for !== undefined) {
                    job.scheduled_for = toRelative(Date.create(job.scheduled_for));
                }
...
//
(function($) {
    var $raw_tpl = $('script[name=scheduler-status]').html();
    var template = _.template($raw_tpl);
    var $container = $('#scheduler-status');
    var $placeholderEl = $('span[data-role=loading-placeholder]', $container);

    var reload_table = function(done) {
        $placeholderEl.show();

        api.getSchedulerStatus(function(data) {
            var el = template({d: data}, {variable: 'd'});
